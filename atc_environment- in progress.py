...
class Environment:
    def __init__(self):
        ...
        self.runway_status = {} # {runway_id: status -> 0/1}
        ...
        class AircraftInteraction(CyclicBehaviour):
            ...
            async def send_instruction_to_atc(self, position,distance):
                ...
                if self.agent.reached_destination:
                    msg.body = f"Aircraft {self.agent.aircraft_id} reached destination.{position}"
...
async def main():
    ...
    aircraft_agent = AircraftAgent("aircraft_agent@localhost", "1234", atc_environment, 100)
